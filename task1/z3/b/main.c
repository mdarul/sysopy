...
int main() {
    ...
    found_list = find_node_in_list(list, list->first->contact);
    timeCheckpoint("Find contact in list (optimisic)");

    found_list = find_node_in_list(list, list->last->contact);
    timeCheckpoint("Find contact in list (pessimistic)");

    found_bst = find_node_in_bst(bst, bst->root->contact);
    timeCheckpoint("Find contact in BST (optimisic)");

    BSTnode *tmp = bst->root;
    while(tmp->right) tmp = tmp->right;
    found_bst = find_node_in_bst(bst, tmp->contact);
